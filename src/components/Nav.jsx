  ...
  return (
    <header className='relative pt-[41px] xl:pt-[56px] font-vietnam'>
      <nav className='flex items-center relative justify-between w-[87.5%] md:w-[77.25%] mx-auto min-h-[28px] xl:w-[77.25%] xl:max-w-[1112px]'>
        ...
        />
        <div className='absolute top-[2px] right-16 lg:right-40 z-20 rounded-full lg:p-2 cursor-pointer' onClick={() => handleTheme()}>
          {dark ? <span className='text-[24px] text-DarkGrayishBlue hover:text-BrightRed duration-300'><FaMoon /></span> : <span className='text-[24px] text-DarkBlue hover:text-BrightRed duration-300'><FaSun /></span>}
        </div>
        <div 
        className='relative flex flex-col min-h-[18px] gap-[3px] w-[26px] lg:hidden'
        onClick={() => setIsOpen(isOpen => !isOpen)}>
          {!dark ? <img src={isOpen ? CloseMenu : Hamburguer} alt="" className='' /> : <img src={isOpen ? CloseMenuDark : HamburguerDark} alt="" className='' />}
        </div>
